...
function CartDAO(database) {
    ...
    this.itemInCart = function(userId, itemId, callback) {
        ...
        this.db.collection("cart").find({"userId":userId, "items._id":itemId}).toArray(function(err, userCart){
            if(userCart[0]['items']){
                for(var i = 0; i < userCart[0]['items'].length; i++){
                    var cartItem = userCart[0]['items'][i];
